...
const Home = () => {
	...
	const flushLeaves = async () => {
		var con = window.confirm(
			"Are you sure you want to flush all leaves from the databse....this action cannot be undone!!"
		);
		if (con) {
			await axios
				.post("https://leavesysbit.pythonanywhere.com/api/flush")
				.then(() => {
					alert("All leaves flushed...Happy New Year!!");
				});
		}
	};
